...
MemPool::initialize()
{
    ...
    if (getenv("TWK_MEM_POOL_SIZE"))          poolSize      = atoi(getenv("TWK_MEM_POOL_SIZE"))*1024*1024;
    if (getenv("TWK_MEM_POOL_MIN_ELEM_SIZE")) minElemSize   = size_t(atof(getenv("TWK_MEM_POOL_MIN_ELEM_SIZE"))*1024.0*1024.0);
    if (getenv("TWK_MEM_POOL_ALLOC_SLOP"))    allocSlop     = atof(getenv("TWK_MEM_POOL_ALLOC_SLOP"));

    globalMemPool = new MemPool(poolSize, minElemSize, allocSlop);

    globalMemPool->m_shortCircuit = (getenv("TWK_MEM_POOL_DISABLE") != 0);

    if (getenv("TWK_MEM_POOL_DEBUG")) globalMemPool->m_debugOutput = true;

    if (globalMemPool->m_debugOutput)
    {
        cerr << "MP: size " << poolSize/(1024*1024) << "MB, minElemSize " << float(minElemSize)/(1024.0*1024.0) << 
                ", slop " << allocSlop << ", shortCircuit " << globalMemPool->m_shortCircuit << endl;
    }
}
...
MemPool::alloc(size_t size)
{
    ...
    else
    {
        ...
        if (!ptr)
        {
            ...
            if (ptr) mp.m_elemMap[ptr] = new PoolElem(mp, ptr, size);
        }
        else hit = true;
    }
...
void
MemPool::FreeList::addElem(PoolElem* elem)
{
    ...
    if (globalMemPool->m_debugOutput)
    {
        cerr << "MP: freelist total " << totalCount << " elems, " << totalSize/(1024*1024) << "MB out of " << poolSize/(1024*1024) << "MB" << endl;
    }
}

void*
MemPool::FreeList::findAndUseElem(size_t size)
{
    ...
    for (PoolElem *i = head; i; i = i->next)
    {
        ...
        {
            //
            //  If we have nothing and this one is within slop allowance, use
            //  it.  If we already have something, use this candidate if it has
            //  less slop.
            //
    ...
    if (candidate)
    {
        ...
        if (candidate == head) head = candidate->next;
        if (candidate == tail) tail = candidate->prev;
