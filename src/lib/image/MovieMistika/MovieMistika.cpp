...
static int
timecodeToFrames(std::string tc, float fps)
{
...
public:
    ...
    char    	timeCode[128];  // null terminated string containing timecode, preferebly as "frame of day", like 90000, but can be 01:20:30:11 style too
    ...
    uint32_t GetBytesPerFrame()
    {
        ...
        switch(packing)
        {
            case DM_PACKING_RGB:         size =  sizeY * sizeX * 3; break;
            case DM_PACKING_RGBA:        size =  sizeY * sizeX * 4; break;
            case DM_PACKING_YUV422:      size =  sizeY * sizeX * 2; break;
            case DM_PACKING_YUV422X2:    size =  sizeY * sizeX * 4; break;
            case DM_PACKING_RGB10:       size =  sizeY * sizeX * 4; break;
            case DM_PACKING_YUV422_10:   size = (sizeY * sizeX * 5) / 2; break;
            case DM_PACKING_YUV422_10X2: size =  sizeY * sizeX * 5; break;
            case DM_PACKING_RGBA_16:     size =  sizeY * sizeX * 8; break;
            case DM_PACKING_RGBA_EXR:    size =  sizeY * sizeX * 8; break;
    ...
    void swapEndian(void* ptr, uint32_t size)
    {
        ...
        { char aux = p[i]; p[i] = p[size - 1 - i]; p[size -1 - i] = aux; }
    }
    ...
    std::string lookupPacking(int packing)
    {
        ...
        switch (packing)
        {
            case 1000: name = "RGB";         break;
            case 1002: name = "RGBA";        break;
            case 1008: name = "YUV422";      break;
            case 1100: name = "YUV422X2";    break;
            case 8001: name = "RGB10";       break;
            case 8100: name = "YUV422_10";   break;
            case 8101: name = "YUV422_10X2"; break;
            case 8200: name = "RGBA_16";     break;
            case 8300: name = "RGBA_EXR";    break;
            default:   name = "Uknonwn";     break;
        }
        return name;
    }
};
...
MovieReader*
MovieMistika::clone() const
{
    MovieMistika* mov = new MovieMistika();
    if (filename() != "") mov->open(filename());
    return mov;
}

void
MovieMistika::open(const string &filename, const MovieInfo &info,
    const Movie::ReadRequest& request)
{
    ...
    {
        TWK_THROW_STREAM(IOException, "Unsupported Mistika packing format: " << m_header->lookupPacking(m_header->packing) << " (" << m_header->packing << ")");
    }
...
void
MovieMistika::imagesAtFrame(const ReadRequest& request, FrameBufferVector& fbs)
{
    ...
    //
    // Get the first frame. The header oocupies the first frame's worth of space
    // so make sure to skip past that.
    //
	...
	file = CreateFileW(
		UNICODE_C_STR(m_filename.c_str()),
		GENERIC_READ,
...
//            fb.restructure(m_header->sizeX, m_header->sizeY, 0, 1, FrameBuffer::PACKED_Y8_Cb8_Y8_Cr8, (unsigned char*)frameB, NULL, FrameBuffer::TOPLEFT);
//            break;
...
//            {
//                case RGB8:
//                    Read8Bit::readRGB8(m_filename, (unsigned char*)frameB, fb, m_header->sizeX, m_header->sizeY, 0, false, false, 0);
//                    break;
//                case RGBA8:
//                    Read8Bit::readRGBA8(m_filename, (unsigned char*)frameB, fb, m_header->sizeX, m_header->sizeY, 0, false, false, 0);
//                    break;
//                case RGB8_PLANAR:
//                default:
//                    FrameBuffer::StringVector chans(3);
//                    chans[0] = "B"; chans[1] = "G"; chans[2] = "R";
//                    fb.restructure(m_header->sizeX, m_header->sizeY, 0, 3, FrameBuffer::UCHAR, (unsigned char*)frameB, &chans, FrameBuffer::BOTTOMLEFT);
////                    Read8Bit::readRGB8_PLANAR(m_filename, (unsigned char*)frameB, fb, m_header->sizeX, m_header->sizeY, 0, m_swap);
//                    break;
//                case RGB10_A2:
//                case RGB16:
//                case RGBA16:
        ...
        case DM_PACKING_RGB10:
		    ...
		    break;
                ...
                default:
                    ...
                    break;
                case RGB10_A2:
                    Read10Bit::readRGB10_A2(m_filename, (unsigned char*)frameB, fb, m_header->sizeX, m_header->sizeY, 0, m_swap, true, (unsigned char *) frameB);
		    doDelete = false;
                    break;
                case RGB16:
...
////                case RGB10_A2:
////                case RGB16:
////                    Read16Bit::readRGB16(m_filename, (unsigned char*)frameB, fb, m_header->sizeX, m_header->sizeY, 0, m_swap, false, 0);
////                    break;
////                case RGBA16:
////                    Read16Bit::readRGBA16(m_filename, (unsigned char*)frameB, fb, m_header->sizeX, m_header->sizeY, 0, true, m_swap, false, 0);
////                    break;
//                default:
//                case RGB16_PLANAR: // Works
//                    fb.restructure(m_header->sizeX, m_header->sizeY, 0, 4, FrameBuffer::USHORT, (unsigned char*)frameB, NULL, FrameBuffer::BOTTOMLEFT);
////                    Read16Bit::readRGB16_PLANAR(m_filename, (unsigned char*)frameB, fb, m_header->sizeX, m_header->sizeY, 0, m_swap);
//                    break;
//            }
//            break;
        default:
...
void
MovieMistika::identifiersAtFrame(const ReadRequest& request,
...
void
MovieMistika::identifier(int frame, ostream& o)
{
...
MovieMistikaIO::MovieMistikaIO() : MovieIO("MovieMistika","v1")
{
...
std::string MovieMistikaIO::about() const
{
    return "Mistika Movie";
}

MovieReader*
MovieMistikaIO::movieReader() const
{
    return new MovieMistika();
}

MovieWriter*
MovieMistikaIO::movieWriter() const
{
    return 0;
}

void
MovieMistikaIO::getMovieInfo(const std::string &filename, MovieInfo&) const
{
